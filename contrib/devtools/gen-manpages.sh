...
LITECOIND=${BITCOIND:-$BINDIR/bitcoinevolutiond}
LITECOINCLI=${BITCOINCLI:-$BINDIR/bitcoinevolution-cli}
LITECOINTX=${BITCOINTX:-$BINDIR/bitcoinevolution-tx}
LITECOINQT=${BITCOINQT:-$BINDIR/qt/bitcoinevolution-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
BTCevVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
